...
- сбрасывать временные логи и выключать `dnsmasq logqueries`, если он остался включен.
...
Стрелки вверх/вниз - выбор   Enter - открыть   q - выход
...
Перед live-сбором скрипт показывает короткую подсказку: для чистого результата лучше открывать проверяемый сайт в инкогнито/приватном окне и сбросить DNS-кеш на устройстве.

Команды ниже выполняются на клиентском устройстве, с которого открывается сайт, а не в SSH-терминале роутера:
...
Пункт меню `Сбросить временные логи`:

- показывает подтверждение перед удалением;
- выключает `dnsmasq logqueries`, если он остался включен после нештатного завершения;
- удаляет `/tmp/podkop-domain-capture.log` и служебные файлы pdc в `/tmp`;
- удаляет временные `/tmp/*domains*.log` и `/tmp/*dns*.log`;
- перезапускает RAM-log через `/etc/init.d/log restart`;
- не очищает DNS-кеш на клиентских ПК/телефонах.
