...
> Не запускайте интерактивное меню через `wget -O - ... | sh`: после pipe меню может остаться без stdin.
...
Пункт `Сбросить временные логи` удаляет последний live-лог и служебные файлы pdc в `/tmp`, перезапускает RAM-log роутера и не очищает DNS-кеш клиентских устройств.
